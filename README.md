...
│   ├── client.ts          # Browser-specific entry
│   ├── server.ts          # Server-specific entry
│   ├── react.ts           # React/TanStack Query integration
│   ├── core.ts            # Core HTTP factory
│   ├── advanced.ts        # Circuit breaker, pools, polling, plugins
│   ├── types.ts           # TypeScript types
│   ├── errors.ts          # Error classes (HttpError, TimeoutError, NetworkError)
│   ├── utils.ts           # Utility functions
│   ├── socket.io.ts       # Realtime entry point
│   └── realtime/
│       ├── index.ts       # Realtime exports & singleton
│       ├── client.ts      # RealtimeClient with logger/hooks/validation
│       ├── types.ts       # Realtime TypeScript types
│       ├── errors.ts      # Realtime error classes
│       ├── hooks.ts       # React hooks for realtime
│       ├── provider.tsx   # React context provider
│       ├── context.ts     # React context
│       ├── csrf-handler.ts# CSRF handler for socket connections
│       └── offline-queue.ts # Offline message queue
├── dist/                  # Compiled ESM + CJS + DTS
├── package.json
├── tsconfig.json
├── tsup.config.ts
└── test.ts                # 21-case test suite
```
...
# Build the package (ESM + CJS + declarations)
bun run build

# Watch mode (auto-rebuild on changes)
bun run dev

# Run tests (21 test cases)
bun run test
```
...
# 4. Publish (public access is pre-configured)
npm publish
```

The `package.json` is pre-configured with:
- `publishConfig.access: "public"` for scoped package publishing
- Proper `exports` map for ESM/CJS/TypeScript resolution
- `files: ["dist"]` to only ship compiled output
